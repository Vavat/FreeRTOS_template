...
/* cunning way to convert character into a digit. This is possible due to ASCII
 * table digit mapping. */
...
UARTCommsController::UARTCommsController()
    : m_pHUART(NULL), m_TxBuffer(200), m_RxBuffer(64), m_ControllerCount(0),
      m_bTestVariable(false) {
...
void UARTCommsController::Initialise(UART_HandleTypeDef *pHUART,
                                     bool debugChannel) {
  m_pHUART = pHUART;
...
void UARTCommsController::Run() {
  ...
  for (;;) {
    ...
    if (bCommandReady) {
      ...
      do {
        *reply = '\0';
        bCommandRecognised = (m_pControllers[controller])
                                 ->NewCommand((char *)m_NewCommand, reply);
...
int UARTCommsController::SendReply(char *pReply) {
  ...
  CheckSum &= 0xF;
  if (CheckSum < 10) {
    m_TxBuffer.Set(CheckSum +
                   48); // offset 0 to 48, which is ascii value for "0".
  } else {
    m_TxBuffer.Set(CheckSum +
                   55); // offset 10 to 65, which is ascii value for "A".
  }
  m_TxBuffer.Set('\r');
  taskEXIT_CRITICAL();
  return CharCounter;
...
void UARTCommsController::TxChar() {
  if (!m_TxBuffer.IsEmpty()) {
    // TODO Need to make this more efficient and transmit the entire buffer
    // contents.
 ...
 * >command 		- simplest command. No nodeID, no CRC. Only for debug
 * purposes.
 * >command;XX 	    - CRC checked command. This is what all clients must be
 * sending. CRC covers everything between starting marker and semicolon.
 */
bool UARTCommsController::ParseCommand(char *pData) {
  ...
  case EParserState_Command:
    ...
    } else if (isCheckSum(*pData)) { /* ';' received. end of command string,
                                        start of checksum */
      *m_pNewCommandChar = '\0';
  ...
  case EParserState_Checksum2:
    if (isxdigit(*pData)) { /* hex digit expected. if checksum fits, command is
                               deemed accepted.*/
      ...
      if (csScan == 1 && m_RxCRC == CalculateChecksum()) {
        ...
        Reset();
        char reply[50];
        sprintf(reply, "Incorrect checksum: %02X. Expected: %02X", m_RxCRC,
                CalculateChecksum());
        SendReply(reply);
      }
    } else {
